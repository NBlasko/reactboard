...
router
  ...
  .get(validateQueryString(schemas.imageQueryIDpublicIDSchema), Imageservice.fetchProfileImage);

router
  .route("/profileimage")
  /*  find image URL in ImagesGallery model and saves it in User model as profile image */
  .post(passportJWT, validateBody(schemas.mongoIdSchema), Imageservice.setProfileImage);

router
  .route("/gallerylist")
  /*  fetches id's of images in ImagesGallery model  */
  .get(passportJWT, validateQueryString(schemas.skipSchema), Imageservice.gallerylist);

router
  .route("/galleryImage")
  /*  grab URL in gallery, then sends an image to <img /> tag in gallery  */
  .get(validateQueryString(schemas.singleGalleryImageSchema), Imageservice.singleGalleryImage)

  /* uploads image in gallery  */
  .post(passportJWT, parser.single("image"), Imageservice.newGalleryImage);

router
  .route("/galleryImage/:id")
  /*  deletes an image with the given id from gallery  */
  .delete(passportJWT, validateParam(schemas.mongoIdSchema, "id"), Imageservice.deleteGalleryImage);
